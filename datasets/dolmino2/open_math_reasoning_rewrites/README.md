...
The raw reasoning traces generated by R1-style models are often (1) quite verbose, (2) contain seemingly useless reasoning paths (e.g., verifying a trivial fact), and (3) may try the same mistaken approach over and over. We thus sought to explore the degree to which these verbose behaviors are useful for downstream training.
We design a spectrum of verbosity-reducing strategies, ranging from "more faithful to the original trace" to "keep only the core content."
